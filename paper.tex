...
Our approach enables better discrimination of cell types \new{and other features of interest} and reduces the need for large numbers of cells.
...
Repeating our analysis on the 63\,421 cCREs with the highest quality came at the cost of a reduced ability to distinguish cell types, suggesting that the average VMR is more informative for this task (Fig.~\ref{fig:resource_usage}A,B).
This demonstrates that our \textit{de novo} VMR detection approach identifies genomic regions in scBS data that are as informative as state-of-the-art epigenomic annotations, as well as its ability to find a parsimonous set of relevant elements. We detected only limited overlap between VMRs and cCREs (Fig.~\ref{fig:score}F), indicating that VMR detection yields information that is complementary to other epigenetic marks.
...
Furthermore, using VMRs over regulatory regions resulted in decreased RAM requirements as well as a shorter runtime, even when accounting for the additional step of VMR detection (Fig.~\ref{fig:resource_usage}C,D).
}
...
We have implemented the approach just described in a Python package with a command line interface, called \texttt{scbs}, which also offers a number of other functionalities for the analysis of scBS data (Fig.~\ref{fig:workflow}).
...
\new{To furthermore aid interpretation of scBS data, we developed and implemented the first algorithm for genome-wide detection of DMRs in single-cell methylomes.}
...
However, one can also use to the same end established tools based on matrix factorisation such as MOFA+, \citep[][included in our benchmarks]{argelaguet2020mofa} or LIGER \citep{welch2019single}.
...
Our implementation can handle datasets of various sizes up to a hundred thousand cells (see Supplementary Fig.~\ref{fig:umaps}B).}
...
\begin{minipage}{\linewidth}
	...
	\new{Analysing this large data set takes approximately a week on a computer with 256 GB RAM and 48 CPUs, with almost all of this time (152 hours) spent by the \texttt{prepare} step, which needs to be run only once per data set. The speed of this step can be increased considerably by processing chromosomes in parallel (each takes 6 to 12 hours depending on its size).}
	}}
	\label{fig:umaps}
\end{minipage}
