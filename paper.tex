...
Specifically, we selected four different sets of genomic intervals at which DNA methylation is to be quantified:
...
Finally, the resulting methylation matrices were subjected to iterative PCA (see Methods).
}
...
\new{
The results demonstrate that quantifying methylation at VMRs or regulatory elements leads to a cleaner separation of cell states than using promoter regions or 100~kb tiles.
...
\new{
We repeated this benchmark on \todo{N} published single-cell methylome data sets.
...
\new{
These extensive benchmarks revealed that our proposed combination of methods, i.e.~using the shrunken means of residuals of VMRs for dimensionality reduction, is able to distinguish diverse cellular properties such as cell type, \todo{colorectal cancer sampling region(?)}, embryonic stage, and germ layer (Fig.~\ref{fig:score_luo}).
In addition, the use of ENCODE regulatory elements as input features also yielded good results.
However, due to their large number (339\,815 in mouse, 1\,063\,878 in human) using regulatory regions as features resulted in greater run times and increased RAM usage, even when accounting for the additional step of VMR detection (Fig.~\ref{fig:score}F).
}
...
For the data of \citet{luo2017single}, results are nearly independent of the parameters (Fig.~\ref{fig:sweep}B).
In the more challenging data set of \citet{kremer_scnmt} (Fig.~\ref{fig:sweep}A), cell types were less cleanly separated when very large bandwidths, or very strict variance thresholds were selected.
...
Overall, however, our default parameter combination provided good results and fast compute times in both data sets.
...
\begin{figure*}
	...
	\caption{\small \textbf{Benchmark of our methods on single-cell multi-omic data of cells of the murine forebrain}.\\
		...
		\textbf{(D)} Illustration of the neighbor score.
        Note that, actually, distances are measured in 15-dimensional PC space, not in the 2-dimensional UMAP space.
		...
		Tiles or VMRs were pre-processed as proposed in this work (shrunken mean of residuals subjected to iterative PCA) or using conventional methods (PCA on
        methylation percentages, lightly imputed as done in \citet{luo2017single}; see Methods for details).
