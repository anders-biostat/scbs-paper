...
The analysis of the large datasets obtained from scBS requires preprocessing to reduce data size, improve signal-to-noise ratio, and provide interpretability.
...
Here, we demonstrate that this coarse-graining approach can lead to signal dilution, and propose improved strategies to identify more informative regions for methylation quantification, and a more accurate quantitation method than simple averaging.
...
$$ t_I = \frac{\mu^\text{A}_I - \mu^\text{B}_I}{\sqrt{v^\text{A}_I/|C_A| + v^\text{B}_I/|C_B|}}.$$
...
For GO enrichment analysis, DMRs with an adjusted p-value $<0.01$ were uploaded to GREAT \citep{mclean2010great} with association rules basal plus extension, 0~bp upstream, 20~kb downstream, 1~Mbp max extension, curated regulatory domains included.
