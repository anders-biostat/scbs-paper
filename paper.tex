...
We also implement an approach to detect differentially methylated regions (DMRs) in scBS data.
DMRs detected between oligodendrocytes and neural stem cells occur near genes related to myelination and stem cell maintenance respectively.
These represent core functions of these cell types, which highlights our ability to detect biologically meaningful DMRs.
...
\textbf{In order to avoid matrix elements with zero count to be transformed to minus infinity, one typically adds a very small ``pseudocount'' (often $10^{-4}$) to the normalized fractions before taking the logarithm.}  % is this even correct? -L
...
\begin{figure}
    \begin{center}
    \includegraphics[width=0.85\columnwidth]{figures/Fig_sliding.png}
    \end{center}
...
To re-analyze scBS data sets by \citet{kremer_scnmt} and \citet{luo2017single}, the genome was first annotated with genomic intervals that were either obtained by tiling the genome into non-overlapping 100~kb tiles, or by acquiring VMRs with \texttt{scbs scan} using the current default options (bandwidth $=2000$, stepsize $=10$, variance threshold $= 0.02$).
VMRs with sequencing coverage in less than 
% WIP
