...
\addcontentsline{toc}{1}{(Title and Abstract)}
\twocolumn[{%
\centering
...
\small
\textit{Abstract} -- Single-cell bisulfite sequencing (scBS) allows for assessing DNA methylation at single-base pair and single-cell resolution. Analyzing the big data sets thus obtained requires preprocessing to reduce data size, improve signal over noise, and provide interpretability. Typically, this is simply done by dividing the genome into large tiles and averaging in these. We show that this coarse-graining dilutes signal, and suggest improved approaches to determine more informative regions to quantify methylation in, and a better quantitation method than simple averaging. We demonstrate how our approach enables better distinguishing of cell types and reduces demands on cell number. We also show that our method of finding variably methylated regions finds biologically meaningful loci. Finally, we present a software tool, \texttt{scbs}, that implements this approach and offers further functionality required to work with scBS data.
\end{minipage}
\vspace{6.5ex}
...
Dividing all cells into four groups according to their level of Ube2l6 expression, it is clearly visible that the promoter of this gene is lowly methylated regardless of gene expression, while methylation at the nearest VMR is greatly decreased in cells with high Ube2l6 expression (Figure \ref{figure:correlation}C).
...
\begin{figure*}
    \begin{center}
        ...
        \caption{\small \textbf{Benchmark of our methods on single-cell methylomes of 16 neuronal subtypes}. \textbf{(A)} Nearest neighbor count score using four different method combinations and a variable number of genomic regions (VMRs or 100~kb tiles) as input to PCA. Exemplary UMAPs for encircled analyses are shown in (C). \textbf{(B)} Same benchmark as in (A), but for a smaller data set consisting of 500 randomly subsampled cells. Exemplary UMAPs for analyses are shown in (D). \textbf{(C)} Two exemplary UMAPs of the full data set and methylation fractions of 100~kb tiles (left) or shrunken means of residuals of VMRs (right) \textbf{(D)} Two exemplary UMAPs of the smaller data set and methylation fractions of 100~kb tiles (left) or shrunken means of residuals of VMRs (right). Data from \citet{luo2017single}.}
        \label{figure:score}
    \end{center}
\end{figure*}
...
Most of this gain in performance can be attributed to the use of VMRs over genomic tiles. The use of the shrunken means of residuals over methylation fractions provides an additional small improvement.
...
\addcontentsline{toc}{1}{(Back matters)}
...
\vspace{1.4ex plus 1ex minus .5ex}
\noindent\hfil\rule{.6\columnwidth}{.2pt}\hfil\vspace{0ex plus 1ex}
...
$$ v_I = \frac{\scriptsize{1}}{\scriptsize{|C_I|}}\sum_{j\in C_I}\left( r_{Ij} - \langle r_{Ij'}\rangle_{j'\in C_I} \right)^2,$$
...
\addcontentsline{toc}{1}{References}
