...
\begin{figure*}[t]
    ...
    Tiles or VMRs were pre-processed as proposed in this work (shrunken mean of residuals subjected to iterative PCA, labeled "pre-processing: ours") or using conventional methods (PCA on lightly imputed methylation percentages, based on \citep{luo2017single}).
...
\section{note to Simon: this is how far I got}   % todo rm

\phantomsection
\subsection{Detecting DMRs between oligodendrocytes and neuroblasts}

\begin{figure*}[t]
    \begin{center}
        \includegraphics[width=.7\textwidth]{figures/Fig_DMRs.png}
    \end{center}
    \caption{\small \textbf{Detection of differentially methylated regions between neural stem cells and oligodendrocytes.}\\
    \textbf{(A)} DMRs detected between neural stem cells (NSCs) and oligodendrocytes (ODs) from \citep{kremer_scnmt} (top) and DMRs detected in the same data with randomly permuted cell labels (bottom, used to estimate FDR and determine adjusted p-values).
...
Next, we used the single-cell multi-omics data set \citep{kremer_scnmt} to assess the ability of our DMR detection approach.
We first selected two cell populations from the murine ventricular-subventricular zone, neuroblasts and oligodendrocytes, and then used the method depicted in Fig.~\ref{fig:dmrscan} to identify regions with differing methylation.
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.9\columnwidth]{figures/SFig_benchmark.png}
    \end{center}
