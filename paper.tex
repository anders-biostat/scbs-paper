...
We obtained results that are qualitatively similar to those reported in \citep{luo2017single}, who quantified mCH in 100~kb tiles, suggesting that our methods also function on mCH data.}

In the more challenging data set of \citet{kremer_scnmt} (Fig.~\ref{fig:sweep}A), cell types were less cleanly separated when very large bandwidths, very strict variance thresholds, \new{or a very large step size was} selected.
...
\begin{minipage}{\linewidth}
    ...
    VMRs were detected with \texttt{scbs} \texttt{scan} using various sliding window bandwidths, variance thresholds, or sliding window step sizes (\textbf{A}, bottom right), and on sub-samples of the full data sets.
    \textbf{(C)} UMAPs obtained when analysing data from \citep{luo2017single} with three different strategies for producing a methylation matrix:
