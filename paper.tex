...
However, constructing a matrix suitable as input for PCA from methylation data is requires deviation from the standard scRNA-seq workflow due to considerable differences in data structure.
...
We identify the windows with the most extreme t statistics, e.g. windows in the 2\% upper and lower tails. We then merge any overlapping windows in the upper tail into larger DMRs, and do the same for windows in the lower tail, and then we re-calculate the t statistic for each larger DMR.
...
By offering these functions, \texttt{scbs} \todo{bridges a gap in the chain of existing tools} that so far hindered practitioners in their data analysis.
...
In conclusion, we have presented powerful improvements to scBS data preprocessing and a software tool that implements these and \todo{bridges a gap in existing tool chains}.
