...
\small
\textit{Abstract} -- Single-cell-bisulfite sequencing (scBS) allows to asses DNA methylation with single-basepair and single-cell resolution. Analysing the big data sets thus obtained requires preprocessing to reduce data size, improve signal over noise, and provide interpretability. Currently, this is simply done by dividing the genome into large tiles and averaging in these. We show that this coarse-graining dilutes signal, and suggest improved approaches to determine informative regions to quantify for and to quantitate methylation in these tiles. We demonstrate how our preprocessing approach helps to better distinguish cell types, reduces demands on cell number, and show that our method of finding variably-methylated regions finds biologically meaningful loci. Finally, we present a software tool, \texttt{scbs}, that implements this approach and offers further functionality requried to work with scBS data.
\end{minipage}
\vspace{6.5ex}
