...
\new{
Overall, VMR quantification yielded results similar to those obtained when quantifying ENCODE regulatory regions.
This demonstrates that our VMR detection approach identifies genomic regions in scBS data that are as informative as state-of-the-art epigenomic annotations.
...
Furthermore, using VMRs over regulatory regions resulted in decreased RAM requirements as well as a shorter runtime, even when accounting for the additional step of VMR detection (Fig.~\ref{fig:score}F).
This is likely due to the large number of annotated regulatory features (339\,815 in mouse, 1\,063\,878 in human) as opposed to the 63\,421 detected VMRs.
Filtering regulatory regions by sequencing coverage reduced resource requirements, but at the cost of a reduced ability to distinguish cell types (Fig.~\ref{fig:todo}).
\todo{simon suggested to stress here that our VMRs are more information-rich than encode regions, because you need fewer VMRs for the same results. We could also mention that encode regions perform poorly in combination with MOFA+ for some reason}
}

\new{
We repeated this benchmark on an additional \todo{3(?)} published single-cell methylome data sets.
These include neuronal sub-types of the murine cortex \citep[][using cell type labels derived from CH-methylation in genomic tiles, instead of CpG-methylation, as ground truth]{luo2017single}, cells isolated from mouse embryos during the onset of gastrulation \citep[][using RNA-derived cell clusters, or alternatively embryonic stage as ground truth]{argelaguet2019gastru} and human colorectal cancer cells \citep[][using \todo{tumor region(?)} as ground truth]{bian2018single}.
...
\new{
These extensive benchmarks confirmed that our proposed combination of methods, i.e.~using the shrunken means of residuals of VMRs for dimensionality reduction, is able to distinguish diverse cellular properties such as cell type, \todo{colorectal cancer sampling region(?)}, embryonic stage, and germ layer (Fig.~\ref{fig:resource_usage}).
Our benchmarks furthermore reveal that groups of cells are more readily separated in large data sets.
As the neighbor score started to plateau at approximately 1000 cells, we propose that this represents a reasonable target cell number for future scBS studies.
}
...
\new{To assess whether our proposed methods are also suitable for the analysis of CH-methylation (mCH), we furthermore performed VMR detection with default options on the mCH-portion of this data set (Fig.~\ref{fig:sweep}C).
We obtained results that are qualitatively similar to those reported in \citet{luo2017single}, who quantified mCH in 100~kb tiles, suggesting that our methods also function on mCH data.}
...
\begin{minipage}{\linewidth}
	...
	For each combination of methods, we quantified the ability to separate ground-truth cell labels reported by the authors in 15-dimensional PCA space (neighbor score).
...
\begin{minipage}{\linewidth}
    \begin{center}
        \includegraphics[width=.85\textwidth]{figures/SFig_parameter_sweep.png}
    \end{center}
    \captionof{figure}{\small \textbf{Effect of VMR detection parameters \new{and methylation context} on separation of cell types.}\\
	\new{
    ...
    \textbf{(D)} UMAPs obtained when analysing data from \citep{luo2017single} with three different strategies for producing a methylation matrix:
    ...
    bottom: shrunken mean of residuals in CpG-VMRs.
    }}
    \label{fig:sweep}
\end{minipage}
