...
\textbf{In order to avoid matrix elements with zero count to be transformed to minus infinity, one typically adds a very small ``pseudocount'' (often $10^{-4}$) to the normalized fractions before taking the logarithm.}  % is this even correct?
...
\phantomsection
\section{Methods (note to Simon: didnt go through this yet)}  % todo rm
...
To re-analyze scBS data from \citet{kremer_scnmt}, single-cell CpG methylation reports were first stored with \texttt{scbs prepare} and then smoothed with \texttt{scbs smooth} using the default bandwidth of 1000~bp.
...
The same analysis was repeated for scBS data from \citet{luo2017single}, and for smaller data sets generated by randomly sub-sampling cells separately from both data sets.
For each data set, we thus obtained four methylation matrices:
either 100~kb tiles or VMRs, quantified either by simple averaging or with the shrunken mean of the residuals.
All four matrices were then subjected to PCA, followed by UMAP with parameters min\_dist $=0.2$, init $=$ spca, and n\_neighbors $=\frac{\sqrt{n}}{1.5}$ where $n$ is the total cell number, to adapt to data sets of different cell numbers.

\subsection{Mean neighbor score} \label{methods:score}
To assess performance of our methods, we employed a score 

we processed mouse CpG methylation data from \citet{luo2017single} with several method combinations and scored each result.
...
% WIP
For all intervals (VMRs and 100~kb tiles), the mean methylation fractions and the shrunken means of the residuals were obtained with \texttt{scbs matrix}.

Regions observed in less than four measured CpGs, or that were measured in less than 10\% of cells, were discarded.
