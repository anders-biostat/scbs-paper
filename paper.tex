...
Therefore, we propose the following approach (Fig.~\ref{vmr}): Divide up the chromosome into many \emph{overlapping} windows, that start at regular multiples of a fixed, small, step size.
...
To enable DMR detection in scBS data, we thus developed an approach that detects DMRs of variable size between two groups of cells, and controls the false discovery rate (Fig.~3):

Similar to the previously described VMR identification approach (Fig.~\ref{vmr}), divide each chromosome into overlapping windows shifted by a small and fixed size (step size) and quantify the methylation of each cell in each window as described (Fig.~\ref{residuals}b).
Next, instead of calculating the variance, obtain the t-statistic % Welch t-test
...
\subsection{???}  % guess we have to rewrite this closing paragraph a little bit, to include both DMRs and VMRs
...
\section{Benchmarks} % re-name this to something more general? Application and benchmarks or something?
...
\begin{figure*}[t]
    \begin{center}
        \includegraphics[width=.8\textwidth]{part_martina/NbvsOlig_DMRs.png}
    \end{center}
...
Figure 7A gives an overview of all identified DMRs and the relation of the adjusted p-value to the number of cells included in a DMR, and the difference in methylation between the cell types.
...
An oligodendrocyte DMR stretching across Mbp was chosen to assess the methylation patterns of an individual DMR (Figure 5B-C).
Mbp is highly expressed in oligodendrocytes as it is essential for the myelination of neuronal axons.
The DMR is highly methylated in neuroblasts and lowly methylated in oligodendrocytes (Figure 7B-C), including more substantial demethylation in oligodendrocytes at the position of Mbp (Figure 5C).
...
GO-term enrichment shows that neuroblast DMRs occur near genes involved in the regulation of neurogenesis, neuron differentiation, and the positive regulation of neural precursor cell proliferation, all of which are essential for neuroblasts (Figure 5D).
...
\begin{figure*}[t]
    ...
    \caption{\small \textbf{Overview of the functionalities implemented in the \texttt{scbs} package.}
        ...
        Identified groups of cells (e.~g.
clusters) can then be scanned for DMRs with \texttt{scbs diff}.
    }
    \label{workflow}
\end{figure*}

We have implemented the approach just described in a Python package with a command line interface, called \texttt{scbs}, which also offers a number of other functionalities for the analysis of scBS data (Figure \ref{workflow}).
