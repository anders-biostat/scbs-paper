...
We also implement an approach to detect differentially methylated regions (DMRs) between groups of cells, and demonstrate its ability to detect biologically meaningful regions near 
...
In order to avoid matrix elements with zero count to be transformed to minus infinity, one typically adds a very small ``pseudocount'' (often $10^{-4}$) to the normalized fractions before taking the logarithm \todo{is this even correct?}.
...
\begin{figure*}
    ...
    \textbf{(C)} \todo{plot this as discussed, as in Fig.~6D} Mean methylation near the gene Htra1 (ENSMUSG00000006205).
...
We denote this count $a^k_j$, where we have chosen $k=\frac{\sqrt{n}}{1.5}$.
...
We then quantified DNA methylation at VMRs and promoters with \citettt{scbs matrix}.
...
Log-normalized expression values reported by \citet{kremer_scnmt} were then correlated with methylation of the genes promoter, or with methylation of the VMR closest to the gene body.
...
We omitted lowly expressed genes (with scRNA-seq counts in $\lt5\%$ of cells) and promoters and VMRs with low scBS coverage (in $\lt10$ cells).
